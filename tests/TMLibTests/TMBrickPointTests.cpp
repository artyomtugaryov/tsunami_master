...
protected:
    ...
    static double x;
    static double y;
    static std::shared_ptr<BrickPoint> m_brickpoint_first;
    static std::shared_ptr<BrickPoint> m_brickpoint_second;
    static std::shared_ptr<BrickPoint> m_brickpoint_third;

};

std::shared_ptr<BrickPoint> TMBrickPointTests::m_brickpoint_first;
std::shared_ptr<BrickPoint> TMBrickPointTests::m_brickpoint_second;
std::shared_ptr<BrickPoint> TMBrickPointTests::m_brickpoint_third;
double TMBrickPointTests::x;
double TMBrickPointTests::y;

TEST_F(TMBrickPointTests, EmptyConstructor) {
    m_brickpoint_first = std::make_shared<BrickPoint>();
    EXPECT_EQ(m_brickpoint_first->x(), 0);
    EXPECT_EQ(m_brickpoint_first->y(), 0);
}

TEST_F(TMBrickPointTests, ConstructorWithParameters) {
    m_brickpoint_first = std::make_shared<BrickPoint>(x, y);
...
TEST_F(TMBrickPointTests, OperatorLESSRetunrFalseWithDifferentX) {
    m_brickpoint_first = std::make_shared<BrickPoint>(x+1, y);
    m_brickpoint_second = std::make_shared<BrickPoint>(x, y-1);
...
TEST_F(TMBrickPointTests, rotate) {
    m_brickpoint_first = std::make_shared<BrickPoint>(x-1, y);
    m_brickpoint_second = std::make_shared<BrickPoint>(x, y-1);
    m_brickpoint_second = std::make_shared<BrickPoint>(x-1, y-1);
    std::cout<<BrickPoint::rotate(*m_brickpoint_first, *m_brickpoint_second, *m_brickpoint_second)<<std::endl;
}
